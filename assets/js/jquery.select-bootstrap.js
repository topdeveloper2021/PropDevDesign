         ...
         if (that.options.liveSearch) {
           $items.each(function (i) {
             if (!$(this).hasClass('disabled')) {
               $(this).data('index.htm', i);
             }
           });
           index = $items.index($items.filter('.active'));
           first = $items.first().data('index.htm');
           last = $items.last().data('index.htm');
           next = $items.eq(index).nextAll().eq(0).data('index.htm');
           prev = $items.eq(index).prevAll().eq(0).data('index.htm');
           nextPrev = $items.eq(next).prevAll().eq(0).data('index.htm');
         }
